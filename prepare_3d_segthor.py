...
def resize_volume(volume: np.ndarray, shape: tuple[int, int, int]) -> np.ndarray:
    factors = (shape[0] / volume.shape[0], shape[1] / volume.shape[1], shape[2] / volume.shape[2])
    return zoom(volume, factors, order=1 if volume.dtype == np.float32 else 0)

def process_patient(id_: str, dest_path: Path, source_path: Path, shape: tuple[int, int, int],
    ...
    resized_ct = resize_volume(norm_ct, shape)
    resized_gt = resize_volume(gt, shape)

    assert resized_ct.shape == shape
    assert resized_gt.shape == shape
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--shape', type=int, nargs=3, default=[512, 512, 200])
