...
class TransformerBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout, mlp_ratio=4.):
        super().__init__()
        self.attention_norm = nn.LayerNorm(embed_dim, eps=1e-6)
        self.mlp_norm = nn.LayerNorm(embed_dim, eps=1e-6)
        self.mlp_dim = int(embed_dim * mlp_ratio)
        self.mlp = Mlp(in_features=embed_dim, hidden_features=self.mlp_dim, act_layer=nn.GELU, drop=dropout)
...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        position_embeddings = nn.Parameter(torch.zeros(1, num_patches, self.embed_dim)).to(x.device)
...
class UNETR(nn.Module):
    ...
    def forward(self, x):
        ...
        patch_dim = [s // self.patch_size for s in x.shape[2:]]
        
        z3 = self.proj_feat(z3, self.embed_dim, patch_dim)
        z6 = self.proj_feat(z6, self.embed_dim, patch_dim)
        z9 = self.proj_feat(z9, self.embed_dim, patch_dim)
        z12 = self.proj_feat(z12, self.embed_dim, patch_dim)
...
def test_unetr():
    ...
    for size in [(128, 128, 128), (200, 200, 200), (256, 256, 256)]:
        print(f"Testing with input size: {size}")
        model = UNETR(input_dim=1, output_dim=2)
